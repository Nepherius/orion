...
interface ProjectionsTabProps {
  creatureList: string[]; // List of creatures the user has hunted to populate the dropdown
...
export default function ProjectionsTab({ 
  ...
  const getVolatilityRisk = (cv: number) => {
    if (cv < 0.2) return { level: 'Low', color: 'text-green-400', desc: 'Return is very consistent across sessions.' };
    if (cv < 0.5) return { level: 'Moderate', color: 'text-yellow-400', desc: 'Expect standard daily swings.' };
    if (cv < 0.8) return { level: 'High', color: 'text-orange-400', desc: 'Spiky returns. Requires larger bankroll.' };
    return { level: 'Extreme', color: 'text-red-400', desc: 'Very inconsistent. Relies heavily on huge globals/HoFs to break even.' };
  };
  ...
  const getTrendColor = (trend: number, trueReturn: number) => {
    ...
    return 'text-yellow-400'; // Normal variance
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="card p-6 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted mt-1">
            Advanced statistical modeling based on your <span className="font-semibold text-primary-400">entire lifetime</span> data (ignores the time range filter).
          </p>
        </div>
      ...
      {/* General Projections & Predictions */}
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="card p-6">
              ...
              <div className="flex items-center gap-2">
                {stats.volatilityCv > 0.5 ? (
                  <AlertTriangle className={`w-6 h-6 ${getVolatilityRisk(stats.volatilityCv).color}`} />
                ) : (
                  <ShieldCheck className={`w-6 h-6 ${getVolatilityRisk(stats.volatilityCv).color}`} />
                )}
                <div className={`text-2xl font-bold ${getVolatilityRisk(stats.volatilityCv).color}`}>
                  {getVolatilityRisk(stats.volatilityCv).level}
                </div>
              </div>
              <div className="text-xs text-muted mt-1">{getVolatilityRisk(stats.volatilityCv).desc}</div>
            </div>
            ...
            <div className="card p-6">
              ...
              <div className="text-2xl font-bold text-blue-400 flex items-center gap-1">
                {stats.cycleToStabilize > 0 && stats.cycleToStabilize < 1000000 
                  ...
                  : stats.cycleToStabilize >= 1000000 ? "1,000,000+" : "Unknown"} PED
              </div>
            ...
            <div className="card p-6">
              ...
              <div className="text-xs text-muted mt-1">
                Assuming ~20 hours hunting/month
              </div>
            </div>
            ...
            <div className="card p-6">
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center justify-between p-3 bg-surface rounded hover:bg-surface-hover">
                  ...
                  <div className={`text-right font-bold text-lg ${getTrendColor(stats.trend50, stats.trueReturnPercent)}`}>
                ...
                <div className="flex items-center justify-between p-3 bg-surface rounded hover:bg-surface-hover">
                  ...
                  <div className={`text-right font-bold text-lg ${getTrendColor(stats.trend10, stats.trueReturnPercent)}`}>
                  ...
                  {stats.fatigueDropoff > 5 ? (
                    <>
                      ...
                      <p className="text-sm text-muted mt-2 max-w-sm">
                        Your short sessions return <span className="text-white font-bold">{Math.abs(stats.fatigueDropoff).toFixed(1)}%</span> MORE than your long sessions. The system might have a dynamic cap hitting you. Try hunting in smaller bursts.
                      </p>
                    </>
                  ) : stats.fatigueDropoff < -5 ? (
                    <>
                      <TrendingUp className="w-12 h-12 text-green-400 mb-3" />
                      <h4 className="font-bold text-lg text-green-400">No, long sessions are better!</h4>
                      <p className="text-sm text-muted mt-2 max-w-sm">
                        Your long sessions return <span className="text-white font-bold">{Math.abs(stats.fatigueDropoff).toFixed(1)}%</span> MORE than your short sessions. Keep grinding, you are building cycle momentum.
                      </p>
                    </>
                  ) : (
                    <>
                      ...
                      <p className="text-sm text-muted mt-2 max-w-sm">
                        Your returns between short bursts and long marathons are practically identical (within {Math.abs(stats.fatigueDropoff).toFixed(1)}%). Play as long as you want!
                      </p>
                    </>
                  )}
                </div>
