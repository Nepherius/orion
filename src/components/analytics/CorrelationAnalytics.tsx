...
import { calculatePearsonCorrelation, calculateMultipleCorrelation } from '../../utils/analyticsCalculations';
...
export function CorrelationAnalytics({ filteredSessions }: CorrelationAnalyticsProps) {
    const correlationData = useMemo(() => {
        // We only want completed sessions that have valid stats for a fair comparison
        ...
        // Calculate individual Pearson correlations against Total Loot
        const durationVsLoot = calculatePearsonCorrelation(durationHrs, lootPed);
        const costVsLoot = calculatePearsonCorrelation(costPed, lootPed);

        // Calculate multiple correlation (R^2) to see if it's a "mix of both"
        const multiple = calculateMultipleCorrelation(durationHrs, costPed, lootPed);
    ...
    const getSignificanceLevel = (p: number) => {
        ...
        if (p < 0.1) return <span className="text-yellow-400">Marginally Significant (p &lt; 0.10)</span>;
        ...
        <div className="card p-6 border border-border">
            <div className="mb-4">
                ...
                <p className="text-sm text-muted mt-1">
                    Statistical analysis of exactly what affects your total loot across {n} completed sessions.
                </p>
            </div>
                ...
                <div className="border border-border rounded p-4 bg-surface-active">
                    ...
                    <div className="flex justify-between items-end mb-2">
                        ...
                        <div className="text-right">
                            ...
                            <div className="text-sm font-semibold">{getCorrelationStrength(durationVsLoot.r)}</div>
                        </div>
                    </div>
