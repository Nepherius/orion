...
pub fn db_get_analytics_advanced_data(
    ...
    let mut creature_stmt = conn
        .prepare(
            ...
            SELECT
                kb.creature,
                COUNT(DISTINCT kb.session_uuid) AS sessions_count,
                COALESCE(SUM(kb.loot_value), 0) AS total_loot,
                COALESCE(SUM(kb.cost), 0) AS total_cost,
                COUNT(*) AS total_kills,
                COALESCE(SUM(CASE WHEN sc.creature = kb.creature THEN COALESCE(gs.globals_count, 0) ELSE 0 END), 0) AS total_globals
            FROM kills_base kb
            LEFT JOIN session_creature sc ON sc.session_uuid = kb.session_uuid
            LEFT JOIN globals_by_session gs ON gs.session_uuid = kb.session_uuid
            GROUP BY kb.creature
